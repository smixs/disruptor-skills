...
Не набор разрозненных советов, а **упорядоченный флоу** для [Claude Code](https://claude.com/claude-code).
...
**Disruptor** — это **Claude Code Skill**: девять выстраданных на реальных проектах методов,
сшитых в **один конвейер**. Каждая стадия принимает артефакт предыдущей и отдаёт следующей, а
**роутер с Yes/No-гейтами** решает, что делать дальше — чтобы агент шёл **по правилу, а не
импровизировал** и не выдумывал «готово» раньше времени.

> **Ключевой принцип:** собственное ощущение агента «готово» — враг. Каждый метод существует,
...
**Для кого:** те, кто собирает софт агентами (Claude Code / Codex) и устал от vibecode —
кода, который «технически работает», но раздут, хрупок и не держится в голове.
...
**On-ramps** (вход не только из идеи): **баг/инцидент** → сразу QA → фикс через движок ·
...
без перезапуска сессии). Отдельная CLI-команда установки не нужна — скиллы обнаруживаются по
...
проект: каждый метод здесь выстрадан на живых прогонах, а не выдуман. Если у тебя есть своя
дисциплина, правка из реального фейла, новый on-ramp или гейт — **присылай PR**.

Как контрибьютить — [CONTRIBUTING.md](CONTRIBUTING.md). Обсуждение, заявки и споры —
